...
\begin{longtable}{|R{1.1cm}|R{2.7cm}|R{2.9cm}|R{2.7cm}|R{2.7cm}|}
...
Desarrollar y evaluar, como prototipo académico, modelos de Machine Learning para la predicción de un indicador de salud pública, la desnutrición crónica infantil (entendida como la clasificación de la condición observada), aplicando la metodología OSEMN a datos estructurados de la ENSANUT 2018 y exponiéndolos mediante una API REST y un tablero web, con evaluación específica para Tungurahua.
...
\subsection{Relación con el tema del trabajo}
El tema del trabajo se refiere a modelos de \textit{Machine Learning} para la predicción de indicadores de salud pública con datos estructurados en Tungurahua. La Tabla~\ref{tab:tema} indica cómo se concreta cada elemento del tema y dónde están sus límites.

\begin{table}[H]
  \caption{Concreción de los elementos del tema en el trabajo.}
  \label{tab:tema}
  \tablabase\small
  \begin{tabular}{|p{3.4cm}|p{10.1cm}|}
    \hline
    \enc{Elemento del tema} & \enc{Concreción y límite en este trabajo}\\ \hline
    Modelos de \textit{Machine Learning} & Se comparan cuatro algoritmos (regresión logística, árbol de decisión, Random Forest y XGBoost) con el mismo protocolo; se selecciona Random Forest.\\ \hline
    Predicción & Se emplea como estimación de la clase de cada niño a partir de sus características (clasificación supervisada binaria). El diseño es transversal: no anticipa casos futuros.\\ \hline
    Indicadores de salud pública & Se concreta en un indicador de la ENSANUT: la desnutrición crónica infantil (retraso en talla para la edad). Otros indicadores de la misma encuesta, como la emaciación o el bajo peso, no se modelan; el flujo podría repetirse para ellos, pero no se evalúa aquí.\\ \hline
    Metodología OSEMN & Se aplicó en sus cinco fases (Capítulo~II).\\ \hline
    Datos estructurados & Microdatos tabulares de la ENSANUT 2018 (114 predictores). La ENDI se exploró, pero no alimenta el modelo.\\ \hline
    En Tungurahua & El modelo se entrena con los registros nacionales y se evalúa por separado en Tungurahua (\num{531} registros; \num{112} en el conjunto de prueba), por lo que esa evaluación se reporta con intervalos de confianza y es incierta.\\ \hline
  \end{tabular}
\end{table}
