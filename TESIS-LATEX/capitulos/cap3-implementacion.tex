...
\begin{table}[H]
  ...
  \begin{tabular}{|p{3.0cm}|p{3.2cm}|p{2.2cm}|p{4.4cm}|}
    ...
    Tablero & React / Vite & 19 / 8 & Interfaz web\\ \hline
...
Las pruebas funcionales incluyeron 17 casos (Anexo~C) y las 22 pruebas automáticas del repositorio. Las 22 pruebas y 16 de los 17 casos se cumplieron; falló CP15 (la puntuación del formulario individual casi no varía con los datos). CP14 (archivo Excel enviado directamente a la API) falló en la versión evaluada inicialmente y se volvió a ejecutar con el servicio corregido (Anexo~C). La latencia de \texttt{POST /predict}, medida en 100 solicitudes en ejecución local (sin red), tuvo mediana de 29 ms, percentil 95 de 33 ms y máximo de 38 ms. Como comprobación de consistencia, la probabilidad devuelta por la API para un registro coincidió con la calculada directamente sobre el modelo (0.437).
...
\begin{itemize}
  ...
  \item \textbf{Sin autenticación (decisión de alcance).} La API no autentica usuarios ni restringe el origen de las solicitudes (CORS abierto). Se decidió no implementarlo porque el modelo se entrena con microdatos públicos de la ENSANUT y el prototipo no está pensado para recibir datos personales. Esa justificación tiene una condición: el servicio guarda en el servidor una copia del resultado de cada archivo que clasifica, por lo que no debe usarse con datos de niños identificables sin antes agregar autenticación y una política de retención de archivos.
  \item \textbf{Defectos de la rama desplegada.} La revisión de \texttt{main} encontró un endpoint de depuración público (\texttt{/api/debug-files}), que mostraba la ruta base del servidor y los nombres de los archivos de datos y de resultados, y un endpoint \texttt{/api/example} que respondía \texttt{null} porque un cambio accidental le había quitado el retorno. Ninguno lo usaba el tablero. Se eliminó el primero y se restauró el segundo, con una prueba de regresión (Anexo~E).
...
\begin{table}[H]
  ...
  \begin{tabular}{|c|c|p{9.4cm}|}
    ...
    RNF4 & No cumple & La API no implementa autenticación y el CORS está abierto. Es una exclusión declarada del alcance: los datos son públicos y el prototipo no debe recibir datos personales (Capítulo~I)\\ \hline
    RNF5 & Cumple & Semilla y versiones fijadas en \texttt{requirements.txt}\\ \hline
    RNF6 & Cumple & Módulos separados y 22 pruebas automáticas aprobadas\\ \hline
    RNF7 & Cumple & Aviso académico visible en todas las pantallas y lenguaje clínico retirado (Anexo~B)\\ \hline
  \end{tabular}
\end{table}

En síntesis, el prototipo cumple 14 de los 15 requerimientos; el restante (RNF4, autenticación) no se implementó por decisión de alcance. El modelo tiene un desempeño moderado (ROC-AUC de 0.677), sobreajuste medido y ninguna validación externa; sus productos son una API funcional con datos completos y un tablero de consulta cuyo formulario individual solo es ilustrativo. Se requiere validar el modelo con una fuente independiente.

\section{Cronograma y Presupuesto}
\subsection{Cronograma de Ejecución}
La Tabla~\ref{tab:cronograma} presenta las actividades ejecutadas.
...
El desarrollo usó software libre y un equipo del investigador. La Tabla~\ref{tab:presupuesto} presenta el presupuesto estimado.
