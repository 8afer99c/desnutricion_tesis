...
\noindent\hspace*{\parindent}\textbf{Objetivo 5: validación funcional y limitaciones.} Las 21 pruebas automáticas y 16 de los 17 casos de prueba se cumplieron (falló la variación de la puntuación del formulario individual), y la latencia mediana fue de 29 ms en ejecución local. Estas pruebas verifican el funcionamiento del software y no la validez predictiva ni clínica. Se identificó que el nivel de riesgo contradecía la clase asignada (corregido: BAJO coincide ahora con la clase 0), que el resumen del tablero usaba predicciones sobre datos de entrenamiento (corregido: ahora usa el conjunto de prueba), que la puntuación del formulario individual casi no varía con los datos porque el resto de las variables se imputa, y que la API no tiene autenticación (exclusión declarada del alcance).
...
\begin{itemize}
  ...
  \item \textbf{Seguridad y reproducibilidad:} la API no autentica usuarios y el CORS está abierto; es una exclusión declarada del alcance, porque los datos son públicos. El servicio guarda en el servidor una copia del resultado de cada archivo clasificado, por lo que no debe recibir datos personales. Los resultados dependen de las versiones de librerías indicadas.
...
\begin{itemize}
  ...
  \item \textbf{Proteger la API si se usa con datos no públicos:} agregar autenticación, restringir el origen de las solicitudes y definir una política de retención de los archivos de resultado.
\end{itemize}
