...
// La API puede responder con claves «*_clase_desnutricion» o con las abreviadas (accuracy, recall...).
const leer = (m: any, ...claves: string[]): number | undefined => {
  for (const c of claves) if (typeof m?.[c] === 'number') return m[c];
  return undefined;
};
...
const Metrics: React.FC = () => {
  ...
  const disponibles = ['recall_clase_desnutricion', 'recall', 'roc_auc', 'pr_auc'].some((c) => (leer(metricas, c) ?? 0) > 0);
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      {metricas && !disponibles && (
        <p className="text-sm text-amber-800 bg-amber-50 border border-amber-200 rounded-2xl p-4">
          Las métricas del modelo no están disponibles en este despliegue (falta el archivo de métricas). Las cifras del informe de tesis
          provienen del conjunto de prueba interno.
        </p>
      )}
      ...
      {metricas && disponibles && (
        <>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card titulo="Sensibilidad" ayuda="De los niños con desnutrición crónica, proporción que el modelo identifica." valor={leer(metricas, 'recall_clase_desnutricion', 'recall')}
                  icono={<AlertCircle className="w-5 h-5" />} color="bg-orange-50 text-orange-600" />
            <Card titulo="Precisión" ayuda="De los niños que el modelo clasifica con desnutrición crónica, proporción que realmente la tiene." valor={leer(metricas, 'precision_clase_desnutricion', 'precision')}
                  icono={<CheckCircle2 className="w-5 h-5" />} color="bg-teal-50 text-teal-600" />
            <Card titulo="F1 (clase con desnutrición)" ayuda="Media armónica entre precisión y sensibilidad de la clase con desnutrición." valor={leer(metricas, 'f1_clase_desnutricion', 'f1_score')}
                  icono={<BarChart3 className="w-5 h-5" />} color="bg-blue-50 text-blue-600" />
            <Card titulo="Exactitud balanceada" ayuda="Promedio de la sensibilidad y la especificidad." valor={leer(metricas, 'accuracy_balanceada', 'accuracy')}
                  icono={<Scale className="w-5 h-5" />} color="bg-indigo-50 text-indigo-600" />
            <Card titulo="ROC-AUC" ayuda="Capacidad de discriminación independiente del umbral (0.5 equivale al azar)." valor={leer(metricas, 'roc_auc')}
                  icono={<TrendingUp className="w-5 h-5" />} color="bg-purple-50 text-purple-600" />
            <Card titulo="PR-AUC" ayuda="Área bajo la curva precisión–sensibilidad. La referencia sin información es la prevalencia (≈ 0.25)." valor={leer(metricas, 'pr_auc')}
