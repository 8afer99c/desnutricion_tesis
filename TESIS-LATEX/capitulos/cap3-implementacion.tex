...
Las pruebas funcionales incluyeron 17 casos (Anexo~C) y las 5 pruebas automáticas de contrato del repositorio. Las 5 pruebas y 15 de los 17 casos se cumplieron; fallaron CP14 (archivo Excel enviado directamente a la API) y CP15 (la puntuación del formulario individual casi no varía con los datos). La latencia de \texttt{POST /predict}, medida en 100 solicitudes en ejecución local (sin red), tuvo mediana de 29 ms, percentil 95 de 33 ms y máximo de 38 ms. Como comprobación de consistencia, la probabilidad devuelta por la API para un registro coincidió con la calculada directamente sobre el modelo (0.437).
...
\begin{itemize}
  \item \textbf{Riesgo y clase no coinciden siempre.} La clase positiva se asigna con el umbral de 0.42, pero los niveles de riesgo usan cortes de 0.50 (medio) y 0.80 (alto). En el caso CP4, la API devolvió clase 1 con probabilidad de 0.437 y riesgo BAJO.
  \item \textbf{El resumen del tablero usa predicciones sobre datos de entrenamiento.} \texttt{GET /dashboard} calcula los ``casos'' como positivos predichos por el modelo sobre el archivo completo (\num{20510} registros, que incluye los usados para entrenar), por lo que no equivale a la prevalencia observada (\SI{24.7}{\percent}).
  ...
  \item \textbf{Carga de archivos Excel.} La pantalla mencionaba archivos \texttt{.xlsx}, pero el servicio solo lee CSV y un Excel enviado a la API produce una excepción no controlada (CP14). El tablero ahora acepta solo CSV y muestra un mensaje claro con las columnas faltantes (CP16); la API aún no maneja el Excel de forma controlada.
...
\begin{table}[H]
  ...
  \begin{tabular}{|c|c|p{9.4cm}|}
    ...
    RF4 & Parcial & CP8: un CSV de 50 registros devuelve 50 filas; CP14: un archivo Excel produce un error no controlado\\ \hline
    RF5 & Cumple & CP11: métricas idénticas a \texttt{metricas\_v2.json}\\ \hline
    RF6 & Parcial & CP9, CP10 y CP12 responden, pero el resumen usa predicciones sobre datos de entrenamiento\\ \hline
    ...
    RNF6 & Cumple & Módulos separados y 5 pruebas automáticas aprobadas\\ \hline
    RNF7 & Cumple & Aviso académico visible en todas las pantallas y lenguaje clínico retirado (Anexo~B)\\ \hline
  \end{tabular}
\end{table}

En síntesis, el prototipo cumple 12 de los 15 requerimientos, 2 parcialmente y 1 no se cumple. El modelo tiene un desempeño moderado (ROC-AUC de 0.677), sobreajuste medido y ninguna validación externa; sus productos son una API funcional con datos completos y un tablero de consulta cuyo formulario individual solo es ilustrativo. Quedan pendientes la autenticación, la corrección en la API del nivel de riesgo, del resumen del tablero y de la lectura de Excel, y la validación con una fuente independiente.
