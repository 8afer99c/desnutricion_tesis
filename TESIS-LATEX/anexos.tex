...
La variable de entorno \texttt{MODEL\_VERSION} selecciona el modelo activo: \texttt{v2} (por defecto) o \texttt{v1}. El umbral de decisión de la versión v2 se lee de \texttt{metricas\_v2.json} (0.42); la versión v1 usa 0.5. El servicio se inicia en Render con \texttt{uvicorn app.main:app --host 0.0.0.0 --port \$PORT}, y el tablero usa la variable \texttt{VITE\_API\_URL} para ubicar la API cuando se despliega en Vercel.

En la rama \texttt{main}, que es la desplegada, la API se expone bajo el prefijo \texttt{/api} (por ejemplo, \texttt{/api/predict}) y el mismo servicio entrega el tablero compilado. En \texttt{pipeline-v2-fixes}, con la que se ejecutaron los casos de prueba del Anexo~C, las rutas no llevan prefijo. En este documento las rutas se escriben sin prefijo; los cambios posteriores a las pruebas (nivel de riesgo, resumen del tablero, lectura de archivos y plantilla de ejemplo) se verificaron también en \texttt{main} con el prefijo \texttt{/api}.
...
La solicitud debe contener exactamente las 114 variables del modelo (\texttt{GET /features} las lista y \texttt{GET /example} devuelve una plantilla con las 114 variables en \texttt{null}). Las variables categóricas se envían como texto con las mismas categorías del entrenamiento y las numéricas como números. Los valores no informados pueden enviarse como \texttt{null}; el \textit{Pipeline} los imputa con la mediana o la moda del entrenamiento.
...
El tablero se usa desde un navegador web actual. En ejecución local se abre en \texttt{http://localhost:5173} con la API activa en \texttt{http://localhost:8000}.
...
\begin{longtable}{|c|c|p{9.8cm}|}
  ...
  2026-09-20 & \texttt{9a30a0d} & Eliminar el endpoint público de depuración \texttt{/api/debug-files} (\texttt{main})\\ \hline
  2026-09-20 & \texttt{b46738f}, \texttt{4b2ce05} & Restaurar \texttt{/api/example} y agregar su prueba de regresión\\ \hline
\end{longtable}
\endgroup
...
\section*{I.7 Nivel de riesgo (\texttt{config.py}, extracto)}
\lstinputlisting[language=Python,basicstyle=\ttfamily\scriptsize,frame=single]{anexos/codigo/config_riesgo.py}
