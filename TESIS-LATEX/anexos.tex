...
\begin{table}[H]
  ...
  \begin{tabular}{|p{4.0cm}|p{9.4cm}|}
    ...
    \texttt{data/} & Archivos CSV usados por el resumen del tablero\\ \hline
    \texttt{tests/} & Pruebas de contrato de la API y de las métricas\\ \hline
  \end{tabular}
\end{table}
...
La clase se asigna con el umbral de 0.42 (probabilidad $\geq$ 0.42: clase 1). El nivel de riesgo se asigna con cortes distintos: ALTO si la probabilidad es $\geq$ 0.80, MEDIO si es $\geq$ 0.50 y BAJO en los demás casos. Esta diferencia produce inconsistencias (Capítulo~III).
...
Muestra el número de registros y cuántos clasifica el modelo con y sin desnutrición crónica, para todo el país o para la provincia elegida en el selector \textit{Ámbito} (Figura~\ref{fig:ui-dashboard}). \textbf{Interpretación:} son clasificaciones del modelo sobre un archivo de datos que incluye registros usados para entrenarlo; no son la prevalencia observada, como aclara el recuadro amarillo de la pantalla.
...
\begin{table}[H]
  ...
  \begin{tabular}{|p{5.0cm}|p{8.4cm}|}
    ...
    Riesgo BAJO con clase estimada positiva (en el Excel) & La clase usa el umbral 0.42 y el riesgo usa cortes de 0.50 y 0.80; se recomienda usar la clase\\ \hline
  \end{tabular}
\end{table}
...
\begin{longtable}{|c|p{2.2cm}|p{2.7cm}|p{2.4cm}|p{3.1cm}|c|}
  ...
  CP4 & Predicción individual & \texttt{POST /predict} con 114 variables & HTTP 200; clase, probabilidad en [0,1] y riesgo & HTTP 200; clase 1, probabilidad 0.437, riesgo BAJO & Pasa (ver nota)\\ \hline
  ...
  CP9 & Resumen nacional & \texttt{GET /dashboard} & HTTP 200 con totales & HTTP 200; \num{20510} registros; \num{10626} casos predichos & Pasa (ver nota)\\ \hline
  CP10 & Resumen por provincia & \texttt{GET /dashboard} con provincia Tungurahua & HTTP 200 con totales & HTTP 200; 580 registros; 293 casos predichos & Pasa (ver nota)\\ \hline
  ...
  CP14 & Archivo Excel en predicción masiva & \texttt{POST /predict-file} con un \texttt{.xlsx} & Resultado o mensaje de error controlado & Excepción no controlada (\texttt{UnicodeDecodeError}) & Falla\\ \hline
...
\noindent\textbf{Notas.} En CP4 la clase asignada (1) y el riesgo (BAJO) no coinciden porque usan cortes distintos. En CP9 y CP10 los ``casos'' son positivos predichos por el modelo sobre un archivo que incluye los datos de entrenamiento, no casos observados. CP14 y CP15 se agregaron al descubrir el comportamiento del servicio de archivos y del formulario; CP16 y CP17 verifican la carga de archivos del tablero corregido (Anexo~B).

\noindent\textbf{Resumen.} De los 17 casos, 15 pasan y 2 fallan (CP14 y CP15). Los casos CP1 a CP13 verifican el contrato de la API con datos completos; CP14 muestra una limitación del servicio de archivos y CP15 la escasa variación del formulario individual.
